...
async function registerForPushNotificationsAsync() {
  ...
  if (Device.isDevice) {
    ...
    console.log(token);
      //preload data of current location before gowing to camera page
      const fetchData = async () => {
        try {
          const loc_data = await preloadData(token.data);
          setPreloadedData(loc_data);
        } catch (error) {
...
// Define a function to pre-load data
const preloadData = async (data) => {
  try {
    ...
    // Handle errors during pre-loading
    throw new Error('Failed to pre-load data: ' + error.message);
  }
};

// Call the pre-load function before navigating to the camera page
const handleAlert = async () => {
  try {
    let loc_data = preloadedData;
    let witness_name = "Unknown User";
    if(user_name !== "Unknown") {
      witness_name = user_name;
    }
    navigation.navigate('CameraRecordPage', { loc_data, witness_name });
  } catch (error) {
...
if (!preloadedData) {
  // Render the splash screen or loading indicator
  return (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
       <ActivityIndicator size="large" color="#FF6100" /> 
       <Text style={{marginTop: 10, fontSize: 16, color: '#555',}}>Loading resources</Text>
    </View>
  );
}
